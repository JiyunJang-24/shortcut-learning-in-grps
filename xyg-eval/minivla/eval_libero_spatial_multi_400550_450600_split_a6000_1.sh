...
base_ckpt_dir=logs/2025-4-16/16-11-29_libero_qwen_pretrain_split/prism-qwen25-dinosiglip-224px+0_5b+mx-libero-90+n1+b16+x7/checkpoints
...
local_log_dir="./experiments-test/logs-50-02-${min_weight1}-${max_weight1}-${task1_id_arr_str}-${min_weight2}-${max_weight2}-${task2_id_arr_str}"
...
for sub_dir in "${ckpt_paths[@]}"; do
    ...
    if [ "${sub_dir}" != "step-005000-epoch-06-loss=0.8679.pt" ] ; then
        continue
    fi
