...
def main(args):
    ...
    for task_id in tqdm.tqdm(task_id_list, desc=f"tasks-{args.libero_task_suite}"):
        ...
        env, task_description = get_libero_env_multi_embodiment(task, "llava", resolution=IMAGE_RESOLUTION, **env_kwargs)
        franka_env, franka_task_description = get_libero_env(task, "llava", resolution=IMAGE_RESOLUTION)
        ...
        for orig_data_key in tqdm.tqdm(orig_data.keys(), desc="episide in tasks"):  # demo_0, demo_1, ..., demo_49, ...
            ...
            for repeat_idx in range(2): # min(demo_repeat_times, 2) -> 2
                ...
                franka_env.set_init_state(orig_states[0])
                ur5_init_state = convert_franka_flat_to_ur5e_flat(franka_env, env, orig_states[0])
                env.set_init_state(ur5_init_state)
                ...
                intrinsic_matrix = CU.get_camera_intrinsic_matrix(env.sim, camera_name, height, width)
                ...
                for _, action in enumerate(orig_actions):
                    ...
                    # print(obs["robot0_gripper_qpos"], "gripper qpos shape:", obs["robot0_gripper_qpos"].shape)
                    # print(obs["robot0_eef_pos"], "eef pos shape:", obs["robot0_eef_pos"].shape)
                    # print(obs["robot0_eef_quat"], "eef quat shape:", obs["robot0_eef_quat"].shape)
            ...
            if sum(done_list) == len(done_list):    # 没有这个条件就放弃这个trajectory了。
                rep_idx = 0
                for done, states, actions, ee_states, gripper_states, joint_states, robot_states, agentview_images, eye_in_hand_images, intrinsics, extrinsics in zip(done_list, states_list, actions_list, ee_states_list, gripper_states_list, joint_states_list, robot_states_list, agentview_images_list, eye_in_hand_images_list, intrinsic_matrix_list, extrinsic_matrix_list):
