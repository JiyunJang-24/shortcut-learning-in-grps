"""
python experiments/robot/libero/regenerate_libero_hdf5_lerobot_dataset_repeat_split_multi_processor_specify_tmp.py \
...
import LIBERO.xyg_scripts.only_rotate_dataset as only_rotate_dataset
...
def process_task(args, task_id, libero_task_suite, shared_values):
    ...
    for orig_data_key in tqdm.tqdm(orig_data.keys(), desc=f"episodes in task {task_id}", leave=False):
        ...
        for repeat_idx in range(2): # min(demo_repeat_times, 2) -> 2
            ...
            viewpoint_rotate = TASK_ROTATE_DICT[task_id]
            ...
            if need_color_change:
                ...
                env = only_rotate_dataset.rotate_camera(env, camera_id=camera_id, camera_name=camera_name, robot_base_name=robot_base_name, 
...
def main(args):
    ...
    repo_name = repo_name + "_specify_tmp"
