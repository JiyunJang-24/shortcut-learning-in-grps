...
python experiments/robot/libero/regenerate_libero_lerobot_island_dataset_clean.py \
    --libero_task_suite libero_spatial \
...
def main(args):
    ...
    repo_name = f"xyg/v-{viewpoint_rotate_min_interpolate_weight:.2f}-{viewpoint_rotate_max_interpolate_weight:.2f}-c-{color_scale_min_interpolate_weight:.2f}-{color_scale_max_interpolate_weight:.2f}-clean"
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite), desc=f"tasks-{args.libero_task_suite}"):
        ...
        for orig_data_key in tqdm.tqdm(orig_data.keys(), desc="episide in tasks"):  # demo_0, demo_1, ..., demo_49, ...
            ...
            cur_env_image = env.sim.render(256, 256, camera_name=camera_name)
            Image.fromarray(cur_env_image[::-1]).save(f'./tmp_dir/cur_env_image.jpg')
            for idx, transparent_object_name in enumerate(transparent_object_name_list):
                ...
                cur_env_image = env.sim.render(256, 256, camera_name=camera_name)
                # save cur env image
                Image.fromarray(cur_env_image[::-1]).save(f'./tmp_dir/{idx:02d}_{transparent_object_name}.jpg')
