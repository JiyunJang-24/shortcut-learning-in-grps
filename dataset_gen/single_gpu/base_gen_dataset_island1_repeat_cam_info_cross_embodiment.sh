...
BUILDER_DIR="${ROOT}/dataset_git/rlds_dataset_builder/LIBERO_Spatial_XYG_cam_info_ur5e"
username="cam_info_ur5e"
...
for i in "${viewpoint_rotate[@]}"; do
    for specify_task_id in "${specify_task_id_list[@]}"; do
        ...
        if [ 1 -eq 1 ]; then
            python "${ROOT}/experiments/robot/libero/regenerate_libero_hdf5_lerobot_dataset_repeat_split_with_cam_info_cross_embodiment.py" \
