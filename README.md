...
4. Release and test OpenVLA-Mini (MiniVLA) code
...
Environment setup for generating LIBERO datasets and training MiniVLA
```bash
git clone https://github.com/Lucky-Light-Sun/shortcut-learning-minivla.git 
...
```bash
cd shortcut-learning-minivla/lerobot
...
Step 1: Download the original LIBERO Object dataset
...
Step 2: Modify camera viewpoints (task-irrelevant factor) to construct dataset islands
...
# Viewpoint distance settings, (you can adjust parameters to generate these datasets)
# [0.375, 0.575] task 0 ; [0.425, 0.625] task 4
# [0.350, 0.550] task 0 ; [0.450, 0.650] task 4
# [0.300, 0.500] task 0 ; [0.500, 0.700] task 4
# [0.250, 0.450] task 0 ; [0.550, 0.750] task 4
...
# Viewpoint distance settings, (you can adjust parameters to generate these datasets)
# [0.200, 0.200] task list={0,1,3,5,8} ; [0.800, 0.800] task list={2,4,6,7,9}
# [0.300, 0.300] task list={0,1,3,5,8} ; [0.700, 0.700] task list={2,4,6,7,9}
# [0.400, 0.400] task list={0,1,3,5,8} ; [0.600, 0.600] task list={2,4,6,7,9}
# [0.450, 0.450] task list={0,1,3,5,8} ; [0.550, 0.550] task list={2,4,6,7,9}
```
...
- Create a file named `.hf_token` in the project root (`shortcut-learning-minivla`) and paste your Hugging Face token into it.
...
```
# IMPORTANT: In the scripts below, set `libero_raw_data_dir` to your dataset path.
bash xyg-train/shortcut-minivla/train_vla_qwen_vq_split_dataset_a6000_200200_800800_-10_90.sh   # OpenVLA-Mini diversity experiment 1
bash xyg-train/shortcut-minivla/train_vla_qwen_vq_split_dataset_a6000_200350_650800_-10_90.sh   # OpenVLA-Mini diversity experiment 2
# Other training scripts for additional experiments are available in the `xyg-train/shortcut-minivla` directory.
...
# IMPORTANT: Set `--dataset.root` to your dataset path.
#    `--dataset.repo_id`: `xyg_20_10_15.0_65.0/v-0.400-0.400_num1`, `xyg_20_10_15.0_65.0/v-0.600-0.600_num5` are the two sub-datasets you generated earlier.
...
```
# `[0.400, 0.400]` is the viewpoint range for sub-dataset 1.
# `[0.600, 0.600]` is the viewpoint range for sub-dataset 2.
# `lerobot/outputs/train/2025-10-09/02-34-42_diffusion/checkpoints` is the checkpoint directory of diffusion policy.
...
- openvla-mini: https://github.com/Stanford-ILIAD/openvla-mini
